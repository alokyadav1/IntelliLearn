    ...
    return (
        <div className="space-y-10">
            <Section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                    <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl shadow-sm">
                        ...
                        <Bold className="text-xl text-slate-900 block mb-2">Amazon EBS</Bold>
                        <P className="text-sm text-slate-600 mb-4 italic">The "External Hard Drive" analogy</P>
                        <P className="text-sm mb-0">Durable, block-level storage that persists independently of the instance's life. Plug it in, unplug it, data survives.</P>
                    </div>
                    <div className="p-6 bg-emerald-50 border border-emerald-100 rounded-2xl shadow-sm">
                        <div className="text-3xl mb-3">🚀</div>
                        <Bold className="text-xl text-emerald-900 block mb-2">Instance Store</Bold>
                        <P className="text-sm text-emerald-700 mb-4 italic">The "RAM Drive" analogy</P>
                        <P className="text-sm mb-0 text-emerald-800">Physically attached NVMe or SSD disks. Blazing fast, but ephemeral—data is gone when power is off.</P>
                    </div>
                </div>
            </Section>
            ...
            <Section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 my-6">
                    <div className="p-4 border border-slate-100 rounded-xl bg-slate-50/50">
                        <Bold className="text-sm block mb-1">AZ Scoped</Bold>
                        <P className="text-xs text-slate-500 mb-0">The volume and instance must be in the same AZ.</P>
                    </div>
                    <div className="p-4 border border-slate-100 rounded-xl bg-slate-50/50">
                        <Bold className="text-sm block mb-1">Network Attached</Bold>
                        <P className="text-xs text-slate-500 mb-0">Not inside the server; it's a virtual drive over the cable.</P>
                    </div>
                    <div className="p-4 border border-slate-100 rounded-xl bg-slate-50/50">
                        <Bold className="text-sm block mb-1">Scalable</Bold>
                        <P className="text-xs text-slate-500 mb-0">You can resize volumes and change performance on the fly.</P>
                    </div>
                </div>
            ...
            <Section>
                ...
                <div className="p-5 border border-red-100 bg-red-50/30 rounded-xl mb-6">
                    <Bold className="text-red-700 block mb-2">❌ When NOT to use Instance Store:</Bold>
                    <BulletList items={[
                ...
                <Bold className="text-emerald-700 block mb-2">✅ Best Use Cases:</Bold>
                <BulletList items={[
            ...
            <Section>
                ...
                <div className="flex flex-col md:flex-row items-center justify-around gap-4 my-8 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                    <div className="text-center">
                        <div className="text-xs font-bold text-slate-400 mb-2 uppercase tracking-wide">Monday</div>
                        <div className="w-24 h-24 bg-indigo-500 rounded-lg flex items-center justify-center text-white font-bold shadow-lg">10 GB</div>
                        <div className="text-[10px] mt-2 text-slate-500">Full Volume Copy</div>
                    </div>
                    <div className="text-slate-300 text-2xl">→</div>
                    <div className="text-center">
                        <div className="text-xs font-bold text-slate-400 mb-2 uppercase tracking-wide">Tuesday</div>
                        <div className="w-16 h-16 bg-indigo-300 rounded-lg flex items-center justify-center text-indigo-900 font-bold shadow-md">500 MB</div>
                        <div className="text-[10px] mt-3.5 text-slate-500">Only Changed Blocks</div>
                    </div>
                    <div className="text-slate-300 text-2xl">→</div>
                    <div className="text-center">
                        <div className="text-xs font-bold text-slate-400 mb-2 uppercase tracking-wide">Wednesday</div>
                        <div className="w-12 h-12 bg-indigo-200 rounded-lg flex items-center justify-center text-indigo-800 font-bold shadow-sm">200 MB</div>
                        <div className="text-[10px] mt-5 text-slate-500">Only Changed Blocks</div>
                    </div>
                </div>
                ...
                <StepList steps={[
                    {
                        ...
                        description: (
                            <CodeBlock language="bash" code={`aws ec2 create-volume --availability-zone ap-south-1a --size 10 --volume-type gp3\naws ec2 attach-volume --volume-id vol-xxx --instance-id i-xxx --device /dev/xvdf`} />
                        )
                    },
                    {
                        title: "Format and Mount",
                        description: (
                            <CodeBlock language="bash" code={`lsblk\nsudo mkfs -t ext4 /dev/xvdf\nsudo mkdir /data\nsudo mount /dev/xvdf /data\ndf -h`} />
                        )
                    },
                    {
                        title: "Make it Persistent",
                        description: (
                            <CodeBlock language="bash" code={`echo '/dev/xvdf /data ext4 defaults,nofail 0 2' | sudo tee -a /etc/fstab`} />
                        )
                    }
                ]} />
            </Section>
