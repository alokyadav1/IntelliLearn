        ...
        <div className="max-w-5xl mx-auto px-8 py-16 animate-entry">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
                ...
                <div className="card p-8 bg-indigo-600 border-none text-white relative overflow-hidden shadow-lg shadow-indigo-200">
                    <div className="absolute -right-16 -top-16 w-32 h-32 bg-white/10 rounded-full blur-2xl" />
                    <h3 className="text-xl font-bold mb-6 tracking-tight relative z-10">Key Components</h3>
                    <ul className="space-y-4 relative z-10">
                        {[
                            { emoji: "🧠", label: "LLM BRAIN", desc: "The reasoning and decision core." },
                            { emoji: "🛠️", label: "TOOL ABSTRACTIONS", desc: "Functions the agent can trigger." },
                            { emoji: "💾", label: "STATE & MEMORY", desc: "Short-term and persistent indexing." },
                        ].map((item) => (
                            <li key={item.label} className="flex items-start">
                                <div className="bg-indigo-500/50 rounded-lg p-2 mr-4 mt-0.5 border border-white/20">{item.emoji}</div>
                                <div>
                                    <strong className="block text-indigo-50 font-bold text-sm label-small tracking-wider mb-1 mt-1">{item.label}</strong>
                                    <span className="text-indigo-100 text-sm">{item.desc}</span>
                                </div>
                            </li>
                        ))}
                    </ul>
            ...
            <section className="mb-12">
                ...
                <div className="card p-0 overflow-hidden group">
                    <div className="p-8 pb-6 border-b border-slate-100 relative">
                        <div className="absolute right-0 top-0 h-full w-1 bg-cyan-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                        <p className="text-slate-600 text-lg leading-relaxed">
                    ...
                    <div className="code-block rounded-none rounded-b-xl px-8 py-6 m-0 border-t-0 border-x-0 border-b-0 shadow-inner overflow-x-auto">
                        <pre className="text-sm">
                            <code><span className="text-slate-400">const</span> <span className="text-rose-400">webSearchTool</span> <span className="text-slate-400">=</span> {"{"}<br />
                                {"  "}<span className="text-cyan-400">name:</span> <span className="text-teal-300">&quot;search_web&quot;</span>,<br />
                                {"  "}<span className="text-cyan-400">description:</span> <span className="text-teal-300">&quot;Performs a web search to gather recent information on a topic.&quot;</span>,<br />
                                {"  "}<span className="text-cyan-400">parameters:</span> {"{"}<br />
                                {"    "}<span className="text-cyan-400">type:</span> <span className="text-teal-300">&quot;object&quot;</span>,<br />
                                {"    "}<span className="text-cyan-400">properties:</span> {"{"}<br />
                                {"      "}<span className="text-cyan-400">query:</span> {"{"} <span className="text-cyan-400">type:</span> <span className="text-teal-300">&quot;string&quot;</span>, <span className="text-cyan-400">description:</span> <span className="text-teal-300">&quot;The search terminology.&quot;</span> {"}"}<br />
                                {"    "},<br />
                                {"    "}<span className="text-cyan-400">required:</span> [<span className="text-teal-300">&quot;query&quot;</span>]<br />
                                {"  "}{"}"}<br />
                                {"}"};
                            </code>
                        </pre>
                    ...
                    <div className="flex flex-col md:flex-row gap-8 items-start">
                        ...
                        <div className="w-full md:w-64">
                            <div className="flex flex-col items-center gap-2 p-6 bg-slate-50 border border-slate-200 rounded-xl relative">
                                ...
                                <div className="absolute inset-y-0 right-2 w-8 border-r-2 border-dashed border-slate-300 rounded-r-3xl my-6 pointer-events-none" />
                            </div>
                        </div>
                    </div>
                </div>
