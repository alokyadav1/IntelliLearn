...
import { buildingAIAgentsModules } from "@/courses/ai-agents/config/building-ai-agents.config";
import { getUserProgress } from "@/app/actions/progress";

const COURSE_ID = "ai-agents";

export default async function BuildingAIAgentsIndex() {
        ...
        <div className="max-w-6xl mx-auto px-8 py-16 animate-entry">
            ...
            <header className="mb-10">
                ...
                <p className="text-xl text-slate-600 max-w-2xl leading-relaxed">
                    A hands-on, step-by-step learning path. From your first LLM API call to deploying multi-agent production systems. Select a topic to start learning.
                </p>
            </header>

            <div className="space-y-12">
                {buildingAIAgentsModules.map((module) => (
                    <section key={module.id} className="card overflow-hidden">
                        <div className="bg-slate-50 border-b border-slate-100 px-8 py-6 flex items-center">
                            <div className="bg-white border border-indigo-100 text-indigo-600 h-10 w-10 min-w-10 rounded-xl flex items-center justify-center mr-4 text-sm font-bold shadow-sm">
                        ...
                        <div className="p-8 pb-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4">
                                {module.topics.map((topic, index) => (
                                    <Link
                                        key={topic.id}
                                        href={`/courses/ai-agents/building-ai-agents/${topic.id}`}
