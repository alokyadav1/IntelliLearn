...
const accentClasses: Record<string, { active: string; dot: string }> = {
    indigo: {
        active: "bg-indigo-50 text-indigo-700 border-indigo-400",
        dot: "bg-indigo-500",
    },
    orange: {
...
export default function ModuleTopicSidebar({
    ...
    return (
        <aside className="w-72 shrink-0 border-r border-slate-200 bg-slate-50/60 h-full flex flex-col overflow-hidden">
            {/* Header */}
            <div className="px-5 pt-6 pb-4 border-b border-slate-200">
                <Link
                    href={backHref}
                    className="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-slate-700 transition-colors font-medium mb-4"
                >
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Current Module</p>
                        ...
                        >
                            ...
                            <span className="shrink-0 mt-0.5 w-5 flex items-center justify-center">
                                {isCompleted ? (
                                    ...
                                    <span className={`text-xs font-mono ${isActive ? "font-bold opacity-100" : "text-slate-400"}`}>
                                        {index + 1}
                                    </span>
                                )}
                            </span>
